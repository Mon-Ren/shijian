---
name: shijian-spark-prairie-fire
description: |
...
**如果条件不具备：** 先创造条件（调用 `shijian-investigation-first` 做调查研究），或者调整方向。
**如果条件具备：** 坚定信心，从小处着手。
...
4. **稳固性** —— 经过验证（shijian-practice-cognition），确认是稳固的
...
1. **评估客观条件**（先于一切行动）：
   ...
   ```
   条件不足 → 先调用 shijian-investigation-first，不急于行动
