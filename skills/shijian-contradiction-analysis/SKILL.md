---
name: shijian-contradiction-analysis
description: |
...
- 技术矛盾 → 调查研究（调用 shijian-investigation-first），实践验证（调用 shijian-practice-cognition）
- 资源矛盾 → 集中兵力解决主要矛盾（调用 shijian-concentrate-forces）
- 多维平衡矛盾 → 统筹兼顾（调用 shijian-overall-planning）
...
**事实锚点要求：** 每个矛盾的判定必须附带具体依据，禁止凭空判断。
- "主要矛盾"的理由必须引用具体事实（文件内容、错误信息、用户反馈原文、数据指标等）
- 矛盾性质的判断必须有依据（"对抗性，因为 X 方案会直接损害 Y 方案的核心功能"）
- 如果没有任何事实锚点，判定为"空转分析"，必须先调用 investigation-first 补充事实
